...
struct RegionOfInterest
{
	...
	RegionOfInterest(const Point2D &center, const int width = 0, const int height = 0)
		: rect(int(center.x - width / 2.f), int(center.y - height / 2.f), width, height)
	{
	}
...
private:
	...
	inline static const float LAMBDA_FPS = 0.3f;
...
private:
	...
	optional<time_point<steady_clock>> time;
...
public:
	Locator(const LocatorParams &params)
	{
		...
		videoCapture.open(params.camID, cv::CAP_ANY);
		...
		if (params.helper)
		{
			...
			cv::createTrackbar("Low H", "Helper", nullptr, 255, [](int pos, void *obj)
							   { *(unsigned char *)obj = pos; }, &helper_low[0]);
			cv::createTrackbar("High H", "Helper", nullptr, 255, [](int pos, void *obj)
							   { *(unsigned char *)obj = pos; }, &helper_high[0]);

			cv::createTrackbar("Low S", "Helper", nullptr, 255, [](int pos, void *obj)
							   { *(unsigned char *)obj = pos; }, &helper_low[1]);
			cv::createTrackbar("High S", "Helper", nullptr, 255, [](int pos, void *obj)
							   { *(unsigned char *)obj = pos; }, &helper_high[1]);

			cv::createTrackbar("Low V", "Helper", nullptr, 255, [](int pos, void *obj)
							   { *(unsigned char *)obj = pos; }, &helper_low[2]);
			cv::createTrackbar("High V", "Helper", nullptr, 255, [](int pos, void *obj)
							   { *(unsigned char *)obj = pos; }, &helper_high[2]);
	...
	const optional<Point2D> locatePixelXY(const cv::Vec3b lower_hsv, const cv::Vec3b upper_hsv, optional<RegionOfInterest> roi)
	{
		cv::Point2i point{};

		if (roi)
		{
			roi.value().ensureWithinImage(_hsv_frame);
			point = {roi.value().rect.x, roi.value().rect.y};
			cv::inRange(_hsv_frame(roi.value().rect), lower_hsv, upper_hsv, _frame_threshold);
		}
		else
		{
		...
		{
			cv::drawContours(_frame, _contours, -1, avgColor, 3, 8, cv::noArray(), INT_MAX, point);
			m = cv::moments(_contours[largest_contour], true);
		}

		if (m.m00 != 0.)
		{
			_pixel_xy.x = float(m.m10 / m.m00 + point.x);
			_pixel_xy.y = float(m.m01 / m.m00 + point.y);
			return _pixel_xy;
		}
	...
	{
		_image_plane_xyz.x = image_plane_uv.x;
		_image_plane_xyz.y = params.d * cosTheta + image_plane_uv.y * sinTheta;
		_image_plane_xyz.z = params.zc + image_plane_uv.y * cosTheta - params.d * sinTheta;
		return _image_plane_xyz;
	}

	const optional<Point3D> imagePlaneXYZToFloor(const Point3D &image_plane_xyz, const float a = 0.f)
	{
		const float denom = params.zc - image_plane_xyz.z;
		if (denom <= 0.f)
		{
			return std::nullopt;
		}
		...
		_world_xyz.x = l * image_plane_xyz.x;
		_world_xyz.y = l * (params.d * cosTheta + image_plane_xyz.y * sinTheta);
		_world_xyz.z = a;

		return _world_xyz;
	}

	const optional<Point3D> locateMarkAndGet(cv::Vec3b lower_hsv, cv::Vec3b upper_hsv, const float a, optional<RegionOfInterest> roi = std::nullopt)
	{
		...
		if (!locatePixelXY(lower_hsv, upper_hsv, roi))
			return std::nullopt;
		...
		if (!imagePlaneXYZToFloor(_image_plane_xyz, a))
			return std::nullopt;
	...
	void print()
	{
		if (params.helper)
		{
			locateMarkAndGet(helper_low, helper_high, 0.f);
		}
		...
		const float seconds = time.has_value() ? duration<float>(steady_clock::now() - time.value()).count() : 1.f;
